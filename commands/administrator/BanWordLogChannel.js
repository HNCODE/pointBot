module.exports = {
  ...
  run: async ({ client, args, message, data: { getGuild } }) => {
    ...
    if (opts === '없음') {
      await client.database.updateGuild(message.guild.id, { $set: { BannedWordLogChannel: '0' } })
      await message.channel.send('✅ 해당 금지어 로그채널을 **없음** 으로 설정하였습니다!')
    } else {
      ...
      if (getChannel.type !== 'text') return message.channel.send('❎ 금지어 로그채널은 오로지 **텍스트 채널**이여야 합니다!')
      await client.database.updateGuild(message.guild.id, { $set: { BannedWordLogChannel: getChannel.id } })
      await message.channel.send(`✅ 해당 서버의 금지어 로그채널을 ${getChannel} 으로 설정하였습니다!`)
    }
  }
}
